...
> **[EXPANDED — sport + location hubs + home index]** The generator now produces three hub families from the same pipeline (`scripts/hubs.js` content, `buildHubDescriptors()` / `writeHubPages()` in `scripts/build-races.js`): **distance** (5), **sport** (asfalto/trail/ciclismo/senderismo, hand-written content) and **location** (20 autonomous communities, content templated from each community's real race count + provinces). 29 hubs × 3 locales = 87 pages, `HUB_MIN=8` gating (26/29 indexable; Ceuta/Melilla + one small community `noindex` + excluded from sitemap). Every hub cross-links to the sport + distance hubs (connected graph), event→distance-hub links already run on race pages, and the **homepage is now a hub index** with three groups — *Explora por distancia / por deporte / por territorio* (top-8 communities by count) — injected per locale. Sitemap: 5,112 URLs. Verified: build, localized JSON-LD (`CollectionPage+ItemList+BreadcrumbList+FAQPage`), browser render of a location hub (Cataluña = 539 races, provinces listed) + the 3-group home index, 0 console errors. **Remaining hub work:** event→sport/location links on race pages; province & sport×location hubs; bespoke editorial for flagship location hubs.
